...
def symlink_relative(src_path: Path, dst_path: Path, hidden=False) -> None:
...
def symlink_dotfile(dotfile_path: Path, dst_path: Path, hidden=False) -> None:
    symlink_relative(__MOUDLE_PATH / dotfile_path, dst_path, hidden)


def symlink_dotfile_with_root(src_path: Path, dst_path: Path, hidden=False) -> None:
    if dst_path.is_dir() and not dst_path.is_symlink():
        dst_path /= ('.' if hidden else '') + src_path.name
    rel_target_path = os.path.relpath(src_path, dst_path.parent)
...
def copy_dotfile(dotfile_path: Path, dst_path: Path, hidden=False) -> None:
    if dst_path.is_dir():
